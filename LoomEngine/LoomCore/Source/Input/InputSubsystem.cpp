...
namespace Loom
{
    ...
    InputContextHandle InputSubsystem::AddContext(const InputContext& context, uint32 priority)
    {
        if (!context.IsValid())
            return InvalidInputContextHandle;

        const InputContextHandle handle = InputRuntime::PushContext(context, priority);
        if (handle == InvalidInputContextHandle)
            return InvalidInputContextHandle;

        m_Contexts.push_back(handle);
        return handle;
    }

    void InputSubsystem::RemoveContext(InputContextHandle handle)
    {
        if (handle == InvalidInputContextHandle)
            return;

        InputRuntime::RemoveContext(handle);
        m_Contexts.erase(std::remove(m_Contexts.begin(), m_Contexts.end(), handle), m_Contexts.end());
    }

    void InputSubsystem::ClearContexts()
    {
        for (const InputContextHandle handle : m_Contexts)
            InputRuntime::RemoveContext(handle);
    ...
    {
        return BindAction(action, trigger, InvalidOwnerID, std::move(callback));
    }

    InputBindingHandle InputSubsystem::BindAction(const InputAction& action, InputTriggerEvent trigger, OwnerID owner, InputActionCallback callback)
    {
        const InputBindingHandle handle = InputRuntime::BindAction(action, trigger, owner, std::move(callback));
    ...
    void InputSubsystem::UnbindAllForOwner(OwnerID owner)
    {
        InputRuntime::UnbindAllForOwner(owner);
    }
